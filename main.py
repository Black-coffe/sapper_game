...
class Minesweeper:
    ...
    # Данный метод создает кнопку для игрового поля
    ...
    # Этот метод рассчитан на подсчет мин, находящихся в соседних клетках
    ...
    # Метод on_click обрабатывает действия игрока при нажатии на клетку игрового поля
    ...
    # Метод check_win проверяет, выиграл ли игрок или нет
    def check_win(self):
        for i in range(self.rows):
            for j in range(self.columns):
